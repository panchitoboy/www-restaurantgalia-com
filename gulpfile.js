...
gulp.task("build:scripts", function (cb) {
  runSequence(
    "build:scripts:main",
    "build:scripts:gallery",
    "build:scripts:reservation",
    cb
  );
});
...
gulp.task("build:scripts:gallery", function () {
  return gulp
    .src([paths.appJsFiles + "/gallery/*.js"])
...
gulp.task("build:scripts:reservation", function () {
  return gulp
    .src([paths.appJsFiles + "/reservation/*.js"])
...
gulp.task("clean:scripts", function (cb) {
  runSequence(
    "clean:scripts:main",
    "clean:scripts:gallery",
    "clean:scripts:reservation",
    cb
  );
});
...
gulp.task("clean:scripts:main", function (cb) {
  del(
    [paths.jekyllDir + scriptsMainOutput, paths.siteDir + scriptsMainOutput],
    cb
  );
});
...
gulp.task("clean:scripts:gallery", function (cb) {
  del(
    [
      paths.jekyllDir + scriptsGalleryOutput,
      paths.siteDir + scriptsGalleryOutput,
    ],
    cb
  );
});
...
gulp.task("clean:scripts:reservation", function (cb) {
  del(
    [
      paths.jekyllDir + scriptsReservationOutput,
      paths.siteDir + scriptsReservationOutput,
    ],
    cb
  );
});
      ...
      responsive(
        {
          ...
          "gallery/*": [
            { width: 400, height: 400, rename: { suffix: "-min" } },
            { rename: { suffix: "-max" } },
          ],
          "bg.jpg": [{ height: 350 }],
          "galia/*.*": [{ width: 450 }],
...
gulp.task("build:jekyll", function () {
  var shellCommand =
    "bundle exec jekyll build --config _config.yml,_app/localhost_config.yml";
  if (config.drafts) {
    shellCommand += " --drafts";
  }
